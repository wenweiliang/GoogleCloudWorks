prefix="WGS_supporting_read.all.LowPass"
suffix="txt"
range="$*"
...
while read lines; do
	...
	Sample=$(echo $lines| awk -F " " '{print $4}')
	Filename=$(echo $lines|awk -F " " '{print $11}'| awk -F "/" '{print $10}' )
	Pair=${LGene}--${RGene}
	LgsPath="gs://dinglab/wliang_fusion/output/all_100kb_2nd_LowPass/${Filename}.${LGene}.txt"
	RgsPath="gs://dinglab/wliang_fusion/output/all_100kb_2nd_LowPass/${Filename}.${RGene}.txt"
	...
	for i in ${range}; do
		LRangeOfLgene=$(echo $lines |awk -F " " -v x=${i} '{split($13, b,  "[:-]"); print b[2]+100000-x}')
		RRangeOfLgene=$(echo $lines |awk -F " " -v x=${i} '{split($13, b,  "[:-]"); print b[3]-100000+x}')
		LRangeOfRgene=$(echo $lines |awk -F " " -v x=${i} '{split($14, c,  "[:-]"); print c[2]+100000-x}')
		RRangeOfRgene=$(echo $lines |awk -F " " -v x=${i} '{split($14, c,  "[:-]"); print c[3]-100000+x}')
